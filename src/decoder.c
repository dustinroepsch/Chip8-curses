...
void decoder_execute_instruction(chip8_state_t *state,
                                 instruction_t instruction) {
  printf("%x ", instruction.full_opcode);
  ...
  case 0x8:
    ...
    case 0x3:
      ...
      break;
    case 0x4:
      printf("Set V[%x] = V[%x] + V[%x], set VF = carry\n", instruction.X,
             instruction.X, instruction.Y);
      ...
      break;
    case 0x5:
      printf("Set V[%x] = V[%x] - V[%x], set VF = NOT borrow\n", instruction.X,
             instruction.X, instruction.Y);
  ...
  case 0xA:
    ...
    break;
  case 0xB:
    printf("Jump to location %x + V[0].\n", instruction.NNN);
    state->pc = state->pc + state->v[0];
    break;
  case 0xC:
    printf("Set V[%x] = random byte AND kk.\n", instruction.X);
    state->v[instruction.X] = (rand() % 256) & instruction.KK;
    state->pc = state->pc + 2;
  case 0xD:
    printf("Display %x-byte sprite starting at memory location I at (V[%x], "
      ...
      for (size_t bit = 0; bit < 8; bit++) {
        if ((current_byte & (1 << (7 - bit))) != 0) {
          size_t screen_offset =
              (instruction.Y + offset) * CHIP8_SCREEN_WIDTH + instruction.X;
          if (state->screen[screen_offset]) {
            collision = true;
          }
          state->screen[screen_offset] = state->screen[screen_offset] ^ 1;
        }
      }
    }
  ...
  case 0xE:
    ...
    case 0x9E:
      ...
      break;
    case 0xA1:
      printf("Skip next instruction if key with the value of V[%x] not is "
             "pressed.\n",
             instruction.X);
      if (!state->keyboard[state->v[instruction.X]]) {
        state->pc = state->pc + 2;
  ...
  case 0xF:
    switch (instruction.KK) {
    case 0x07:
      printf("Set V[%x] = delay timer value.\n", instruction.X);
      state->v[instruction.X] = state->delay_timer;
      state->pc = state->pc + 2;
      break;
    case 0x0A:
      printf("Wait for a key press, store the value of the key in V[%x].\n",
             instruction.X);
      // TODO
      break;
    case 0x15:
      printf("Set delay timer = V[%x].\n", instruction.X);
      state->delay_timer = state->v[instruction.X];
      state->pc = state->pc + 2;
      break;
    case 0x18:
      printf("Set sound timer = V[%x]\n", instruction.X);
    ...
    case 0x1E:
      ...
      break;
    case 0x29:
      printf("Set I = location of sprite for digit V[%x]\n", instruction.X);
      state->I = 5 * state->v[instruction.X];
      state->pc = state->pc + 2;
      break;
    case 0x33:
      printf("Store BCD representation of V[%x] in memory locations I, I+1, "
             "and I+2",
             instruction.X);
